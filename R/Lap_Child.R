...
{
  str_ini <- str
  lap <- stringr::str_replace_all(str@PGF, str@Param, str@Param)
  for (i in 2:length(code))
  {
    ...
    if (str2@type == "Mother")
      ini <- stringr::str_replace_all(str2@PGF, str2@Param, str2@Param)
    else
      ini <- stringr::str_replace_all(str2@Laplace, str2@Param, str2@Param)
